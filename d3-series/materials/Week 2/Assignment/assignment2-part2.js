...
// Calculate the average weight (lbs) and height (in inches) of the pokemon.
// Note you will have to convert the units to lbs and inches. 

const avgPokeWeight = pokemonData.reduce((acc, curr) => {
    return acc + (parseFloat(curr.weight.substring(0, curr.weight.length - 3)) * 2.205);
 }, 0);

const avgPokeHeight = pokemonData.reduce((acc, curr) => {
  return acc + (parseFloat(curr.height.substring(0, curr.height.length - 2)) * 39.37);
}, 0);
...
var eggDistance = 0;
const pokePsychic = pokemonData.filter(poke => poke.weaknesses.includes('Psychic') && poke.egg != 'Not in Eggs');
const totalDistance = pokePsychic.map(poke=> eggDistance += parseFloat(poke.egg.slice(0, poke.egg.length - 3)));

// Break the pokemon down into 5 equal buckets of weight classes
// Then average the spawn_time (show in minutes) of each weight class
...
pokemonData.forEach(poke => {
  pokeWeights.push(parseFloat(poke.weight.substring(0, poke.weight.length - 3)))
})
pokeWeights.sort((a, b) => a - b);
pokemonData.forEach(poke => {
  if (parseFloat(poke.weight.substring(0, poke.weight.length - 3)) >= 0 && parseFloat(poke.weight.substring(0, poke.weight.length - 3)) <= 92) {
    pokeWeightClass.class1.push(poke)
  } else if (parseFloat(poke.weight.substring(0, poke.weight.length - 3)) > 92 && parseFloat(poke.weight.substring(0, poke.weight.length - 3)) <= 184) {
    pokeWeightClass.class2.push(poke)
  } else if (parseFloat(poke.weight.substring(0, poke.weight.length - 3)) > 184 && parseFloat(poke.weight.substring(0, poke.weight.length - 3)) <= 276) {
    pokeWeightClass.class3.push(poke)
  } else if (parseFloat(poke.weight.substring(0, poke.weight.length - 3)) > 276 && parseFloat(poke.weight.substring(0, poke.weight.length - 3)) <= 368) {
    pokeWeightClass.class4.push(poke)
  } else {
    pokeWeightClass.class5.push(poke)
  }
})